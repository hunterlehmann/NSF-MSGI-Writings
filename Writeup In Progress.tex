...
\begin{document}
	...
	To do this, we introduce the Fock space over $\C^G$ with associated raising and lowering ladder operators $a^j_{mn}, a^{j\dagger}_{mn}$, where $j$ indexes an irrep of $G$, $m$ indexes the copy of the irrep appearing in \cref{eq:reg_rep_decomp}, and $n$ indexes the basis vectors of that copy.
	...
	We can now express the remaining operators in terms of the ladder operators.
	\begin{proposition}[{\cite[Prop. 3]{ZoharBurrello15}}]\label{prop:Uj_ops}
		...
		where $J$ and $K$ are irreps of $G$ and $\braket{J,M,j,m}{K,N}, \braket{K, N'}{J,M,j,m'}$ are Clebsch-Gordon coefficients.
	\end{proposition}
	...
	First, recall that the Clebsch-Gordon coefficients describe a change of basis between the tensor product basis of two irreps of $G$ and the decomposition of that tensor product into irreps.
	...
	Additionally, we are always working with unitary representations and so we can take real Clebsch-Gordon coefficients so that $\braket{J,M,j,m}{K,N}=\braket{K,N}{J,M,j,m}$.
	...
	It follows that the Clebsch-Gordon coefficients are $\braket{J,1}{K}=\delta_{J+1,K}$.
	...
	\begin{equation}
		g\cdot U^j = D^j(g^{-1})U^j \qquad U^j \cdot g = U^j D^j(g).
	\end{equation}
	...
	\noindent TODO: add discussion of Gauss Law?\\
	...
	This would explain both the dimension of the matrix and why there are Clebsch-Gordon coefficients appearing.
	...
	First, we need to understand the Clebsch-Gordon coefficients and thus the tensor products of irreps of $D_3$. 
	...
	\begin{align}
		...
		V^p \otimes V^p &\cong V^p \qquad V^p \otimes V^2 \cong V^2 \qquad V^2 \otimes V^2 \cong V^I \oplus V^p \oplus V^2.
	\end{align}
	The relevant Clebsch-Gordon coefficients are then (taken from \cite{ZoharBurrello15} referencing \cite{vDBC78}):
	\begin{align}
	...
	The other Clebsch-Gordon coefficients appearing in \cref{prop:Uj_ops} are all 0.
	...
	\end{align}
	We can also express these as matrices with respect to the ordered basis $\{ \ket{I},\ket{p},\ket{200},\ket{201},\ket{210},\ket{211} \}$ of the link subspace.
	\begin{align}
