...
\begin{document}
	...
	From these computations we can see that $m(\tau,h)$ is a smooth function of $h$ and so the 1+0 Ising model does not have a phase transition.
	...
	Again we take a lattice $\cL\subset \Z^d$ of $N$ sites, usually cubic.
	...
	Now we can take the hyperplane slices of the lattice along one dimension (the temporal dimension) and consider the integral operator $\hat{T}$ defined by 
		\[ \hat{T}f(\vec{x})=\int_{\R^{d-1}} L(\vec{x},\vec{y})f(\vec{y}) \, d\vec{y},  \]
	where $\vec{x},\vec{y} \in \R^{d-1}$ and $L(\vec{x},\vec{y})$ describes the interactions between hyperplanes.
	We define 
		\[ L(\vec{x},\vec{y})=e^{-\frac{1}{2}S(\vec{x}^2+\vec{y}^2)+\frac{1}{4}\beta J(\vec{x}^TM\vec{x}+\vec{y}^TM\vec{y})+\beta J\vec{x}\vec{y}+\frac{1}{2}\beta h \sum_{i=1}^{d-1} (x_i + y_i)}, \]
	where $M$ is a symmetric matrix describing the spin-spin interactions.
	This lets us write $Q_N(S)=\Tr T^n$, where $n=N/(d-1)$. As in our earlier examples, we now want to extract a quantum Hamiltonian from this formulation by making appropriate choices of couplings and letting the temporal lattice spacing $\tau$ tend to 0.
	
	We will need to make use of the identity
	...
	General concept of gauge invariance/lattice gauge theories.
