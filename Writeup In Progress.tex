...
\begin{document}
	...
	From this result, we can derive the $N\to \infty$ limit of the quantities of interest above, in particular the average magnetization.
	...
	\begin{align*}
		m(\tau, h)&=\lim_{N\to \infty}\frac{\del}{\del h}\left( \frac{1}{N}\log(\lambda_0^N+\lambda_1^N) \right) \\
			&=\frac{\del}{\del h}\lim_{N\to \infty}\left( \frac{1}{N}\left(\log(\lambda_0^N)+\log(1+\left(\frac{\lambda_1}{\lambda_0}\right)^N)\right) \right) \\
			&= \frac{\del}{\del h} \log(\lambda_0),
	\end{align*}
	since $|\lambda_1/\lambda_0| <1.$
	In our example case,
	\begin{align*}
		m(\tau,h)&= \frac{1}{\cosh(\tau h)+ \sqrt{\tau^2 +\sinh^2(\tau h)}}\left( \tau \sinh(\tau h)+\frac{\tau \sinh(\tau h)\cosh(\tau h)}{\sqrt{\tau^2 +\sinh^2(\tau h)}} \right)\\
		&=\frac{\tau \sinh(\tau h)}{\sqrt{\tau^2+\sinh(\tau h)}}.
	\end{align*}
	...
	Detailed analysis of the spherical model in various dimensions can be found in \cite{FradkinSusskind78},\cite{HenkelHoeger84},\cite{Thompson68}.
